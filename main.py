      ...
      for it in range(num_iterations):
          ...
          if(it%opt.display_ratio==0):
              ...
              if(opt.gen_type=='baseline'):
                  ...
                  out, out_2, out_3 = model.gen(input.cuda(), warps.float().cuda(), masks.cuda())
                  images = pose_utils.display(input, target, out.data.cpu(), opt.use_input_pose, opt.pose_dim)
                  images_2 = pose_utils.display(input, target, out_2.data.cpu(), opt.use_input_pose, opt.pose_dim)
                  images_3 = pose_utils.display(input, target, out_3.data.cpu(), opt.use_input_pose, opt.pose_dim)
              ...
              title = "epoch_{0}_{1}_2.png".format(str(epoch).zfill(3), str(it).zfill(5))
              plt.imsave(os.path.join(opt.output_dir, 'test', title), images_2, cmap=plt.cm.gray)
              title = "epoch_{0}_{1}_3.png".format(str(epoch).zfill(3), str(it).zfill(5))
              plt.imsave(os.path.join(opt.output_dir, 'test', title), images_3, cmap=plt.cm.gray)
