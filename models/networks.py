...
class decoder(nn.Module):
    def __init__(self, nfilters_dec, nfilters_enc, num_skips = 1):
        ...
        self.linear_2 = nn.Linear(3 * 32 * 16, 3 * 128 * 64)
        self.linear_3 = nn.Linear(3 * 64 * 32, 3 * 128 * 64)
    ...
    def build_net(self, nfilters_dec):
        ...
        for i, nf in enumerate(nfilters_dec):
            if i==0:
                ...
                if i == 2:
                    self.model_dec_2 = Block((self.num_skips)*self.nfilters_enc[-(i+1)] + nfilters_dec[i - 1], 3, down=False, leaky=False, dropout=True)
                ...
                # due to skip connections
                if i == 3:
                    self.model_dec_3 = Block((self.num_skips)*self.nfilters_enc[-(i+1)] + nfilters_dec[i - 1], 3, down=False, leaky=False, dropout=True)
    ...
    def forward(self, skips):
        for i in range(len(self.nfilters_dec)):
            if (i == 0):
                ...
                if i == 2:
                    out_2 = torch.cat([out, skips[-(i+1)]], 1)
                    out_2 = self.model_dec_2(out_2)
                if i == 3:
                    out_3 = torch.cat([out, skips[-(i+1)]], 1)
                    out_3 = self.model_dec_3(out_3)
        ...
        out = self.net[-1](out)
        out_2 = out_2.view(out_2.shape[0], out_2.shape[1] * out_2.shape[2] * out_2.shape[3])
        out_2 = self.linear_2(out_2)
        out_2 = out_2.view(*out.shape)

        out_3 = out_3.view(out_3.shape[0], out_3.shape[1] * out_3.shape[2] * out_3.shape[3])
        out_3 = self.linear_3(out_3)
        out_3 = out_3.view(*out.shape)



        # out_2 = nn.functional.interpolate(out_2, size=out.shape[2:])
...
class Deformable_Generator(nn.Module):
    ...
    def forward(self, input, warps, masks):
        ...
        out, out_2, out_3 = self.decoder(skips)
        return out, out_2, out_3
    ...
    def forward(self, input, target_pose, target_warps, target_masks):
        ...
        for i in range(self.num_stacks):
            if(i==0):
                ...
                out, out_2, out_3 = self.generator(inp, target_warps[:,i], target_masks[:,i])
            else:
                if(self.use_input_pose):
                ...
                out, out_2, out_3 = self.generator(stage_inp, target_warps[:,i], target_masks[:,i])
            outputs.append(out)
